package com.google.android.gms.internal;

import android.app.Activity;
import android.content.Context;
import android.text.TextUtils;
import android.util.DisplayMetrics;
import android.util.Log;
import android.view.MotionEvent;
import android.view.View;
import java.lang.reflect.InvocationTargetException;
import java.lang.reflect.Method;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
import java.util.concurrent.Callable;
import java.util.concurrent.ExecutorService;
import java.util.concurrent.TimeUnit;

public class zzct extends zzcs {
    private static final String TAG = zzct.class.getSimpleName();
    private static boolean initialized = false;
    private static long startTime = 0;
    private static volatile zzdm zzagq = null;
    private static Object zzahp = new Object();
    protected boolean zzahq = false;
    private String zzahr;
    private boolean zzahs = false;
    private boolean zzaht = false;
    private zzdu zzahu;

    protected zzct(Context context, String str, boolean z) {
        super(context);
        this.zzahr = str;
        this.zzahq = z;
    }

    private static zzdr zza(zzdm zzdm, MotionEvent motionEvent, DisplayMetrics displayMetrics) {
        Method zza = zzdm.zza("LLWEGIcMoDZoYXR7U5oWQiG0eqgHAi5y99iNxWUoUCrqQe4SBuROX6hcHHgPH+SM", "O3KEK4C8mMJ55X7+rKhyFI2gabDgFgd1qlmX9Na34/M=");
        if (zza == null || motionEvent == null) {
            throw new zzdj();
        }
        try {
            return new zzdr((String) zza.invoke(null, new Object[]{motionEvent, displayMetrics}));
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new zzdj(e);
        }
    }

    protected static synchronized void zza(Context context, boolean z) {
        synchronized (zzct.class) {
            if (!initialized) {
                startTime = System.currentTimeMillis() / 1000;
                zzagq = zzb(context, z);
                initialized = true;
            }
        }
    }

    private static zzdm zzb(Context context, boolean z) {
        if (zzagq == null) {
            synchronized (zzahp) {
                if (zzagq == null) {
                    zzdm zza = zzdm.zza(context, "fmyYtvp6GdfV8aXECqySf5usPZLp4lFIXsdmCOa6f3I=", "", z);
                    if (zza.isInitialized()) {
                        zza.zza("b0nS9elqzY5/VJmQt0NlC62dB7yCaB6LeKUB+YXUuuDDycP369v3LvQySmqqDQ6y", "vfxPDbj+Gh4UJloJgP7FDEw0pUJBChFkLoiSG9W3S+I=", Context.class);
                        zza.zza("Nt7RB2lfn/Td3UbDXq8jAOuCI/Ll2Hp2FhUggwUDXFsb8F7bope53TUJgWthHbTZ", "PQ5SSaxTSuJzj647muMyUtZ+uYgTQgJ31c5fk186jfk=", Context.class);
                        zza.zza("xjAQSkwzRxajmgCbKHDMDKALp7GNWm8BXcaw0/hWyFOZu/eOjfjL582/zkzt4p+j", "YBerbJehoFKiWWatcGr+va4R/rOgEqbq5xzJrvD/KqU=", Context.class);
                        zza.zza("Yt5XpSAtwgzkLrmXQYrSRr3fMYrlcB9+S6GNMxeM/YPVx9v5CKvi8nPmgPLSujhj", "dR638FFygwsrTt/QflyOMGvXjf1GSNZ7znIlrDruhqQ=", Context.class);
                        zza.zza("dn8+8pHjJ2wj5Gp1r/T7fEkOwzyz7KKq5AugUUrNOwEHnTcvhTP4W3g7fxLqTpFB", "c6GziBvAtJVQl8145kYNIiXglYZwc9tWD7nqr7hLy/A=", Context.class);
                        zza.zza("aPsscg+Qxy0iqv59xAu19ukICD9tJAt4EQOJODpj5Ak49GynbXKvHyrrGvZ8bZTH", "gvEkKyx3YjkW3LmeffZWM+wAodTj/mcVSvq1iQ03Xm0=", Context.class);
                        zza.zza("VN0mjb52HdSTqOivTj5aML5bEfMpu2JdQlvUopb/cybtBLaro7TgCOYdeQg7NyAj", "gp0cIgUUdAIZbRXtUI5gQGevY7Ql5nC5BzKn6LnObLY=", Context.class);
                        zza.zza("cma5ZMQhKxelkLoCs6AZIf+jHy8LfWoLKTm3I25QQ7y8l7CIShnEfyEmDBtS6X1R", "8HMIy/Wfi2fsqmlkEx1MOaO07gLN9KPbPeJd9GTSqvM=", Context.class);
                        zza.zza("LLWEGIcMoDZoYXR7U5oWQiG0eqgHAi5y99iNxWUoUCrqQe4SBuROX6hcHHgPH+SM", "O3KEK4C8mMJ55X7+rKhyFI2gabDgFgd1qlmX9Na34/M=", MotionEvent.class, DisplayMetrics.class);
                        zza.zza("Ncqp5H+22w9W3L8txFxikPL++CvvT4UACmJT2y9HNWRLTaCz/GhnD086ih+pDclQ", "oClLCKzkLLWAqjtGJeQeLZr6xaB9RrnEuNDXspc7PYI=", MotionEvent.class, DisplayMetrics.class);
                        zza.zza("Be4XNeKe48otW84OnyuJ3t3qbzO8QDmRqoj9GLjPbqCKHI4EjzEK31ZhyeIA63g7", "rw94Vrx/byjpqg0QbJ3qk6x9nbH4tHcDnVkhwVzqmY0=", new Class[0]);
                        zza.zza("fuI1JBLz73BbSdCY+eHqDP1u6baqogxj7PqSFpzZgrAG3XsVxk7kH3pFFSmoNQQw", "o4gCRP8pd3YBLYpyJe+pyrNh2wHBW5ZmIwpxvbEaiEk=", new Class[0]);
                        zza.zza("AifE6OWHQ8zRmArdaoYy3/wDT3Tse3b0SsXzLMY2JhYTdkCeDJ1ty8MUlcSgr6P8", "cpk+vnXicdAmu6rqNpyRsH8Z9xFtWGhKh1PSt7R1wgo=", new Class[0]);
                        zza.zza("0MHmFOerm+OLO7DlwBWYWOfS31i3abNb7NII/p/cozDrwy0Qt4Hz6KjLmcNA2Uu1", "dd/st6q19rXcYPAfnaAl9NStsvK8rw8184B9GIFsJ/U=", new Class[0]);
                        zza.zza("Kl5uWPRlOPd31gHSoBTxQgFz93Wgsh3JQ5+syp0rqA1C1BUyIJ1EfI70uCpXJEXY", "MmO0rCvC3l83tbAoIwnFF0x6ASHVe9zZRiunMzgf61w=", new Class[0]);
                        zza.zza("Jr72ymQHv5OslUBKT3bZCPBk5tCtKWt432zERI2guaDk8CCuhfLPuPGiolFM9O2V", "zYGiKyXCQARtrMz4+R+cwjpghR9+uCtdZ+gYsXH0O9M=", new Class[0]);
                        zza.zza("fCahZmmokRlkTziAR2h88rL/W8OULXQVU2c0PgTQTnRvhkE51fPDmWahg/1J8rwq", "xZw7LXXqVYeXIr/mYvF7euX+S5ZW/jdzGMuluL/FTHw=", Context.class, Boolean.TYPE, Boolean.TYPE);
                        zza.zza("wflZ5ZrFWxKN35Rs/obkm9xcWW4uwhgDI7R93vb2gpXPjsYJYEKVt9jrMQOUQeAA", "cXUR3QZnvsd8QFJrigAEcnhF5qRi6A1WjhzghuHtPIs=", StackTraceElement[].class);
                        zza.zza("nlNsfD0KfLAOzjZnsadcRaT7rHxwp4GC5EZVTVhnwe/pnADxonobSxOiLY3KGgZG", "yEGznoCvboNomg+D/tjSW+eddhf/oFTqSBHDCsg5uVM=", View.class, DisplayMetrics.class);
                        zza.zza("MT46CBRZPOofnotcmzgUNA75mBeAcqcqzo80kU1Wki/M3T0+Ilf2BPD/W1SRuWf8", "85aVi3SJCzURUruw80xwOCJlrH71CKBJVp0z1HeuVQI=", Context.class, Boolean.TYPE);
                        zza.zza("olucaRzp8OAM7j17mlu+MNg8QWI1/0HYxPp/zXgvUcGiOa2hMytoVkpTSVdUA6zj", "kX7NAbLZCz/dSKM5Fw5tfFEDlfsFCqwNHFXHrnBBf44=", View.class, Activity.class);
                        zza.zza("/SomwDyLd+iYT1TBJ8yczLZbKQsVJsVm19/eLPmi+tCXN71809kfhdY0YXdgoZoi", "xQNz5sgiknpAv6ackbFAd0cOAtMz1Xn/IL6OG4Bwh/4=", Long.TYPE);
                    }
                    zzagq = zza;
                }
            }
        }
        return zzagq;
    }

    private static void zzb(List<Callable<Void>> list) {
        if (zzagq != null) {
            ExecutorService executorService = zzagq.getExecutorService();
            if (!(executorService == null || list.isEmpty())) {
                try {
                    executorService.invokeAll(list, ((Long) zzlc.zzio().zzd(zzoi.zzbri)).longValue(), TimeUnit.MILLISECONDS);
                } catch (InterruptedException e) {
                    Log.d(TAG, String.format("class methods got exception: %s", new Object[]{zzds.zza(e)}));
                }
            }
        }
    }

    /* Access modifiers changed, original: protected|final */
    public final long zza(StackTraceElement[] stackTraceElementArr) {
        Method zza = zzagq.zza("wflZ5ZrFWxKN35Rs/obkm9xcWW4uwhgDI7R93vb2gpXPjsYJYEKVt9jrMQOUQeAA", "cXUR3QZnvsd8QFJrigAEcnhF5qRi6A1WjhzghuHtPIs=");
        if (zza == null || stackTraceElementArr == null) {
            throw new zzdj();
        }
        try {
            return new zzdk((String) zza.invoke(null, new Object[]{stackTraceElementArr})).zzail.longValue();
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new zzdj(e);
        }
    }

    /* Access modifiers changed, original: protected|final */
    public final zzba zza(Context context, View view, Activity activity) {
        List arrayList;
        zzba zzba = new zzba();
        if (!TextUtils.isEmpty(this.zzahr)) {
            zzba.zzcu = this.zzahr;
        }
        zzdm zzb = zzb(context, this.zzahq);
        int i = 0;
        if (zzb.isInitialized()) {
            try {
                zzdr zza = zza(zzb, this.zzagw, this.zzaho);
                zzba.zzdm = zza.zzajl;
                zzba.zzdn = zza.zzajm;
                zzba.zzdo = zza.zzajn;
                if (this.zzahn) {
                    zzba.zzea = zza.zzfl;
                    zzba.zzeb = zza.zzfj;
                }
            } catch (zzdj unused) {
            }
            zzbb zzbb = new zzbb();
            if (this.zzagy > 0 && zzds.zza(this.zzaho)) {
                zzbb.zzft = Long.valueOf(zzds.zza(this.zzahf, this.zzaho));
                if (((Boolean) zzlc.zzio().zzd(zzoi.zzbro)).booleanValue()) {
                    zzbb.zzfu = Long.valueOf(zzds.zza((double) (this.zzahk - this.zzahi), this.zzaho));
                    zzbb.zzfv = Long.valueOf(zzds.zza((double) (this.zzahl - this.zzahj), this.zzaho));
                    zzbb.zzfy = Long.valueOf(zzds.zza((double) this.zzahi, this.zzaho));
                    zzbb.zzfz = Long.valueOf(zzds.zza((double) this.zzahj, this.zzaho));
                }
                if (((Boolean) zzlc.zzio().zzd(zzoi.zzbrp)).booleanValue() && this.zzahn && this.zzagw != null) {
                    long zza2 = zzds.zza((double) (((this.zzahi - this.zzahk) + this.zzagw.getRawX()) - this.zzagw.getX()), this.zzaho);
                    if (zza2 != 0) {
                        zzbb.zzfw = Long.valueOf(zza2);
                    }
                    zza2 = zzds.zza((double) (((this.zzahj - this.zzahl) + this.zzagw.getRawY()) - this.zzagw.getY()), this.zzaho);
                    if (zza2 != 0) {
                        zzbb.zzfx = Long.valueOf(zza2);
                    }
                }
            }
            try {
                zzdr zzb2 = zzb(this.zzagw);
                zzbb.zzdm = zzb2.zzajl;
                zzbb.zzdn = zzb2.zzajm;
                zzbb.zzfo = zzb2.zzajn;
                if (this.zzahn) {
                    zzbb.zzfj = zzb2.zzfj;
                    zzbb.zzfl = zzb2.zzfl;
                    zzbb.zzfn = Integer.valueOf(zzb2.zzajo.longValue() != 0 ? 1 : 0);
                    if (this.zzagz > 0) {
                        double d;
                        double d2;
                        Long valueOf;
                        if (zzds.zza(this.zzaho)) {
                            d = (double) this.zzahe;
                            d2 = (double) this.zzagz;
                            Double.isNaN(d);
                            Double.isNaN(d2);
                            valueOf = Long.valueOf(Math.round(d / d2));
                        } else {
                            valueOf = null;
                        }
                        zzbb.zzfk = valueOf;
                        d = (double) this.zzahd;
                        d2 = (double) this.zzagz;
                        Double.isNaN(d);
                        Double.isNaN(d2);
                        zzbb.zzfm = Long.valueOf(Math.round(d / d2));
                    }
                    zzbb.zzfq = zzb2.zzfq;
                    zzbb.zzfp = zzb2.zzfp;
                    zzbb.zzfr = Integer.valueOf(zzb2.zzajr.longValue() != 0 ? 1 : 0);
                }
            } catch (zzdj unused2) {
            }
            if (this.zzahc > 0) {
                zzbb.zzfs = Long.valueOf(this.zzahc);
            }
            zzba.zzer = zzbb;
            if (this.zzagy > 0) {
                zzba.zzef = Long.valueOf(this.zzagy);
            }
            if (this.zzagz > 0) {
                zzba.zzee = Long.valueOf(this.zzagz);
            }
            if (this.zzaha > 0) {
                zzba.zzed = Long.valueOf(this.zzaha);
            }
            if (this.zzahb > 0) {
                zzba.zzeg = Long.valueOf(this.zzahb);
            }
            try {
                int size = this.zzagx.size() - 1;
                if (size > 0) {
                    zzba.zzes = new zzbb[size];
                    while (i < size) {
                        zzdr zza3 = zza(zzagq, (MotionEvent) this.zzagx.get(i), this.zzaho);
                        zzbb zzbb2 = new zzbb();
                        zzbb2.zzdm = zza3.zzajl;
                        zzbb2.zzdn = zza3.zzajm;
                        zzba.zzes[i] = zzbb2;
                        i++;
                    }
                }
            } catch (zzdj unused3) {
                zzba.zzes = null;
            }
            arrayList = new ArrayList();
            if (zzb.getExecutorService() != null) {
                int zzab = zzb.zzab();
                arrayList.add(new zzef(zzb, zzba));
                zzdm zzdm = zzb;
                zzba zzba2 = zzba;
                arrayList.add(new zzei(zzdm, "AifE6OWHQ8zRmArdaoYy3/wDT3Tse3b0SsXzLMY2JhYTdkCeDJ1ty8MUlcSgr6P8", "cpk+vnXicdAmu6rqNpyRsH8Z9xFtWGhKh1PSt7R1wgo=", zzba2, zzab, 1));
                arrayList.add(new zzed(zzdm, "Be4XNeKe48otW84OnyuJ3t3qbzO8QDmRqoj9GLjPbqCKHI4EjzEK31ZhyeIA63g7", "rw94Vrx/byjpqg0QbJ3qk6x9nbH4tHcDnVkhwVzqmY0=", zzba2, startTime, zzab, 25));
                int i2 = zzab;
                arrayList.add(new zzec(zzdm, "0MHmFOerm+OLO7DlwBWYWOfS31i3abNb7NII/p/cozDrwy0Qt4Hz6KjLmcNA2Uu1", "dd/st6q19rXcYPAfnaAl9NStsvK8rw8184B9GIFsJ/U=", zzba2, i2, 44));
                arrayList.add(new zzeh(zzdm, "dn8+8pHjJ2wj5Gp1r/T7fEkOwzyz7KKq5AugUUrNOwEHnTcvhTP4W3g7fxLqTpFB", "c6GziBvAtJVQl8145kYNIiXglYZwc9tWD7nqr7hLy/A=", zzba2, i2, 12));
                arrayList.add(new zzej(zzdm, "aPsscg+Qxy0iqv59xAu19ukICD9tJAt4EQOJODpj5Ak49GynbXKvHyrrGvZ8bZTH", "gvEkKyx3YjkW3LmeffZWM+wAodTj/mcVSvq1iQ03Xm0=", zzba2, i2, 3));
                arrayList.add(new zzeg(zzdm, "Kl5uWPRlOPd31gHSoBTxQgFz93Wgsh3JQ5+syp0rqA1C1BUyIJ1EfI70uCpXJEXY", "MmO0rCvC3l83tbAoIwnFF0x6ASHVe9zZRiunMzgf61w=", zzba2, i2, 22));
                arrayList.add(new zzeb(zzdm, "Yt5XpSAtwgzkLrmXQYrSRr3fMYrlcB9+S6GNMxeM/YPVx9v5CKvi8nPmgPLSujhj", "dR638FFygwsrTt/QflyOMGvXjf1GSNZ7znIlrDruhqQ=", zzba2, i2, 5));
                arrayList.add(new zzeq(zzdm, "VN0mjb52HdSTqOivTj5aML5bEfMpu2JdQlvUopb/cybtBLaro7TgCOYdeQg7NyAj", "gp0cIgUUdAIZbRXtUI5gQGevY7Ql5nC5BzKn6LnObLY=", zzba2, i2, 48));
                arrayList.add(new zzdy(zzdm, "cma5ZMQhKxelkLoCs6AZIf+jHy8LfWoLKTm3I25QQ7y8l7CIShnEfyEmDBtS6X1R", "8HMIy/Wfi2fsqmlkEx1MOaO07gLN9KPbPeJd9GTSqvM=", zzba2, i2, 49));
                arrayList.add(new zzen(zzdm, "Jr72ymQHv5OslUBKT3bZCPBk5tCtKWt432zERI2guaDk8CCuhfLPuPGiolFM9O2V", "zYGiKyXCQARtrMz4+R+cwjpghR9+uCtdZ+gYsXH0O9M=", zzba2, i2, 51));
                arrayList.add(new zzem(zzdm, "wflZ5ZrFWxKN35Rs/obkm9xcWW4uwhgDI7R93vb2gpXPjsYJYEKVt9jrMQOUQeAA", "cXUR3QZnvsd8QFJrigAEcnhF5qRi6A1WjhzghuHtPIs=", zzba2, i2, 45, new Throwable().getStackTrace()));
                arrayList.add(new zzer(zzdm, "nlNsfD0KfLAOzjZnsadcRaT7rHxwp4GC5EZVTVhnwe/pnADxonobSxOiLY3KGgZG", "yEGznoCvboNomg+D/tjSW+eddhf/oFTqSBHDCsg5uVM=", zzba2, i2, 57, view));
                if (((Boolean) zzlc.zzio().zzd(zzoi.zzbrk)).booleanValue()) {
                    arrayList.add(new zzel(zzb, "MT46CBRZPOofnotcmzgUNA75mBeAcqcqzo80kU1Wki/M3T0+Ilf2BPD/W1SRuWf8", "85aVi3SJCzURUruw80xwOCJlrH71CKBJVp0z1HeuVQI=", zzba, zzab, 61));
                }
                if (((Boolean) zzlc.zzio().zzd(zzoi.zzbrl)).booleanValue()) {
                    arrayList.add(new zzdx(zzb, "olucaRzp8OAM7j17mlu+MNg8QWI1/0HYxPp/zXgvUcGiOa2hMytoVkpTSVdUA6zj", "kX7NAbLZCz/dSKM5Fw5tfFEDlfsFCqwNHFXHrnBBf44=", zzba, zzab, 62, view, activity));
                }
                if (((Boolean) zzlc.zzio().zzd(zzoi.zzbrm)).booleanValue()) {
                    arrayList.add(new zzeo(zzb, "/SomwDyLd+iYT1TBJ8yczLZbKQsVJsVm19/eLPmi+tCXN71809kfhdY0YXdgoZoi", "xQNz5sgiknpAv6ackbFAd0cOAtMz1Xn/IL6OG4Bwh/4=", zzba, zzab, 53, this.zzahu));
                }
            }
        } else {
            zzba.zzds = Long.valueOf(16384);
            arrayList = Arrays.asList(new Callable[]{new zzef(zzb, zzba)});
        }
        zzb(arrayList);
        return zzba;
    }

    /* Access modifiers changed, original: protected|final */
    public final zzba zza(Context context, zzax zzax) {
        zzba zzba = new zzba();
        if (!TextUtils.isEmpty(this.zzahr)) {
            zzba.zzcu = this.zzahr;
        }
        zzdm zzb = zzb(context, this.zzahq);
        if (zzb.getExecutorService() != null) {
            zzb(zza(zzb, zzba, null));
        }
        return zzba;
    }

    /* Access modifiers changed, original: protected */
    public List<Callable<Void>> zza(zzdm zzdm, zzba zzba, zzax zzax) {
        int zzab = zzdm.zzab();
        ArrayList arrayList = new ArrayList();
        if (zzdm.isInitialized()) {
            zzdm zzdm2 = zzdm;
            zzba zzba2 = zzba;
            arrayList.add(new zzea(zzdm2, "fCahZmmokRlkTziAR2h88rL/W8OULXQVU2c0PgTQTnRvhkE51fPDmWahg/1J8rwq", "xZw7LXXqVYeXIr/mYvF7euX+S5ZW/jdzGMuluL/FTHw=", zzba2, zzab, 27, zzax));
            arrayList.add(new zzed(zzdm2, "Be4XNeKe48otW84OnyuJ3t3qbzO8QDmRqoj9GLjPbqCKHI4EjzEK31ZhyeIA63g7", "rw94Vrx/byjpqg0QbJ3qk6x9nbH4tHcDnVkhwVzqmY0=", zzba2, startTime, zzab, 25));
            int i = zzab;
            arrayList.add(new zzei(zzdm2, "AifE6OWHQ8zRmArdaoYy3/wDT3Tse3b0SsXzLMY2JhYTdkCeDJ1ty8MUlcSgr6P8", "cpk+vnXicdAmu6rqNpyRsH8Z9xFtWGhKh1PSt7R1wgo=", zzba2, i, 1));
            arrayList.add(new zzek(zzdm2, "xjAQSkwzRxajmgCbKHDMDKALp7GNWm8BXcaw0/hWyFOZu/eOjfjL582/zkzt4p+j", "YBerbJehoFKiWWatcGr+va4R/rOgEqbq5xzJrvD/KqU=", zzba2, i, 31));
            arrayList.add(new zzep(zzdm2, "fuI1JBLz73BbSdCY+eHqDP1u6baqogxj7PqSFpzZgrAG3XsVxk7kH3pFFSmoNQQw", "o4gCRP8pd3YBLYpyJe+pyrNh2wHBW5ZmIwpxvbEaiEk=", zzba2, i, 33));
            arrayList.add(new zzdz(zzdm2, "Nt7RB2lfn/Td3UbDXq8jAOuCI/Ll2Hp2FhUggwUDXFsb8F7bope53TUJgWthHbTZ", "PQ5SSaxTSuJzj647muMyUtZ+uYgTQgJ31c5fk186jfk=", zzba2, i, 29));
            arrayList.add(new zzeb(zzdm2, "Yt5XpSAtwgzkLrmXQYrSRr3fMYrlcB9+S6GNMxeM/YPVx9v5CKvi8nPmgPLSujhj", "dR638FFygwsrTt/QflyOMGvXjf1GSNZ7znIlrDruhqQ=", zzba2, i, 5));
            arrayList.add(new zzeh(zzdm2, "dn8+8pHjJ2wj5Gp1r/T7fEkOwzyz7KKq5AugUUrNOwEHnTcvhTP4W3g7fxLqTpFB", "c6GziBvAtJVQl8145kYNIiXglYZwc9tWD7nqr7hLy/A=", zzba2, i, 12));
            arrayList.add(new zzej(zzdm2, "aPsscg+Qxy0iqv59xAu19ukICD9tJAt4EQOJODpj5Ak49GynbXKvHyrrGvZ8bZTH", "gvEkKyx3YjkW3LmeffZWM+wAodTj/mcVSvq1iQ03Xm0=", zzba2, i, 3));
            arrayList.add(new zzec(zzdm2, "0MHmFOerm+OLO7DlwBWYWOfS31i3abNb7NII/p/cozDrwy0Qt4Hz6KjLmcNA2Uu1", "dd/st6q19rXcYPAfnaAl9NStsvK8rw8184B9GIFsJ/U=", zzba2, i, 44));
            arrayList.add(new zzeg(zzdm2, "Kl5uWPRlOPd31gHSoBTxQgFz93Wgsh3JQ5+syp0rqA1C1BUyIJ1EfI70uCpXJEXY", "MmO0rCvC3l83tbAoIwnFF0x6ASHVe9zZRiunMzgf61w=", zzba2, i, 22));
            arrayList.add(new zzeq(zzdm2, "VN0mjb52HdSTqOivTj5aML5bEfMpu2JdQlvUopb/cybtBLaro7TgCOYdeQg7NyAj", "gp0cIgUUdAIZbRXtUI5gQGevY7Ql5nC5BzKn6LnObLY=", zzba2, i, 48));
            arrayList.add(new zzdy(zzdm2, "cma5ZMQhKxelkLoCs6AZIf+jHy8LfWoLKTm3I25QQ7y8l7CIShnEfyEmDBtS6X1R", "8HMIy/Wfi2fsqmlkEx1MOaO07gLN9KPbPeJd9GTSqvM=", zzba2, i, 49));
            arrayList.add(new zzen(zzdm2, "Jr72ymQHv5OslUBKT3bZCPBk5tCtKWt432zERI2guaDk8CCuhfLPuPGiolFM9O2V", "zYGiKyXCQARtrMz4+R+cwjpghR9+uCtdZ+gYsXH0O9M=", zzba2, i, 51));
            if (((Boolean) zzlc.zzio().zzd(zzoi.zzbrk)).booleanValue()) {
                arrayList.add(new zzel(zzdm, "MT46CBRZPOofnotcmzgUNA75mBeAcqcqzo80kU1Wki/M3T0+Ilf2BPD/W1SRuWf8", "85aVi3SJCzURUruw80xwOCJlrH71CKBJVp0z1HeuVQI=", zzba, zzab, 61));
            }
            return arrayList;
        }
        zzba.zzds = Long.valueOf(16384);
        return arrayList;
    }

    /* Access modifiers changed, original: protected|final */
    public final zzdr zzb(MotionEvent motionEvent) {
        Method zza = zzagq.zza("Ncqp5H+22w9W3L8txFxikPL++CvvT4UACmJT2y9HNWRLTaCz/GhnD086ih+pDclQ", "oClLCKzkLLWAqjtGJeQeLZr6xaB9RrnEuNDXspc7PYI=");
        if (zza == null || motionEvent == null) {
            throw new zzdj();
        }
        try {
            return new zzdr((String) zza.invoke(null, new Object[]{motionEvent, this.zzaho}));
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new zzdj(e);
        }
    }

    public final void zzb(View view) {
        if (((Boolean) zzlc.zzio().zzd(zzoi.zzbrm)).booleanValue()) {
            zzdu zzdu = this.zzahu;
            if (zzdu == null) {
                this.zzahu = new zzdu(zzagq, view);
            } else {
                zzdu.zzd(view);
            }
        }
    }
}
